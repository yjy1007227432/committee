    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/committee_check/checkpersonparameter/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.receiveUnfinished = data.checkpersonParameter.receiveUnfinished
                this.dataForm.receiveNotFollow = data.checkpersonParameter.receiveNotFollow
                this.dataForm.receiveEffect = data.checkpersonParameter.receiveEffect
                this.dataForm.receiveDeduct = data.checkpersonParameter.receiveDeduct
                this.dataForm.receivePut = data.checkpersonParameter.receivePut
                this.dataForm.drivingVisit = data.checkpersonParameter.drivingVisit
                this.dataForm.drivingManage = data.checkpersonParameter.drivingManage
                this.dataForm.drivingUnion = data.checkpersonParameter.drivingUnion
                this.dataForm.notSatisfied = data.checkpersonParameter.notSatisfied
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/committee_check/checkpersonparameter/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
